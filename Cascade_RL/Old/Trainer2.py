...
# --- 3. Environment Components (Generator, Subcircuits, Optimizer) ---
# These are the same as the previous example, so they are abbreviated here.
class Generator:
    omega_c_range = np.logspace(1e2, 1e6, 125)
    middle_zeta_range = np.linspace(0.5, 1.5, 75)
    upper_zeta_range = np.linspace(1.6, 2, 25)
    lower_zeta_range = np.linspace(0.1, 0.4, 25)

    zeta_range = np.concatenate((lower_zeta_range, middle_zeta_range, upper_zeta_range))
    def getCircuit(self, freq):
        """Generate a random circuit's response with stageDecider deciding the number of stages."""
        return self.getSingleCircuit(5, freq)
...
SUBCIRCUITS = [{"name":"SKLP","func":Filter().LP,"p":1}, {"name":"SKHP","func":Filter().HP,"p":1}, {"name":"BSKBS","func":Filter().BRCBS,"p":2}]
class Optimizer:
    def optimize(self, fn, p, err, f): 
        obj = lambda params: np.mean((fn(params, f) - err)**2);
        res, _ = cma.fmin2(obj, p, 0.5, options={'verbose':-9}); return res
