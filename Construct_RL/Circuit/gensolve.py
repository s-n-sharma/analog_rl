...
while counter < TOTAL_LEN:
    for i in range(1, MAX_NODES):
        for _ in range(min(100, i*5)):
            with open(f'temp.txt', 'w') as f:
                ...
                if _ < 60:
                    ...
                    for op_amp_cnt in range(OP_AMP_NUMBER):
                        ...
                        #.subckt iop1 1 2 3
                        f.write(f"E{op_amp_cnt} {node_1} {node_2} {node_3}\n")
            ...
            try:
                circ_obj = circ.Circuit()
                circ_obj.parse_netlist('temp.txt')

                # Generate transfer function for Vin to Vout
                freqs = np.logspace(0, 6, num=100)  # Frequencies from 1Hz to 1MHz
                response = circ_obj.get_transfer_function('Vin', '2', freqs)

                fail_flag = False
                for __ in range(len(freqs)):
                    if response[__] == np.inf or response[__] == -np.inf or np.isnan(response[__]):
                        fail_flag = True
                        break
                if fail_flag:
                    continue
                ...
                # Save the circuit and response
                with open(f'responses/response_{counter}.txt', 'w') as out:
                    for __ in range(len(freqs)):
                        out.write(f'{freqs[__]} {response[__]}\n')
                
                with open(f'circuits/circuit_{counter}.txt', 'w') as out:
                ...
                print("Error generating circuit/response"+str(e))    
                continue
...
k = 10
with open('closest.txt', 'w') as close:
    with open('furthest.txt', 'w') as far:
        for i in range(len(complete)):
            resp = complete[i]
            dots = []
            for j in range(len(complete)):
                if i == j:
                    continue
                dots.append(np.dot(resp, complete[j])/(norms[i] * norms[j]))
            
            dots = np.array(dots)

            idx_closest = np.argpartition(dots, k)[:k]
            idx_furthest = np.argpartition(dots, -k)[-k:]
            close_str = f"{i},"
            ...
            far_str = f"{i},"
            for j in idx_furthest:
                far_str += f"{j},"
            far_str += "\n"

            close.write(close_str)
            far.write(far_str)
